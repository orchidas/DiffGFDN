...
def eq_freqs(interval: int = 1,
             start_freq: float = 31.25,
             end_freq: float = 16000):
    r"""
    ...
    **Returns**:
    ...
    center_freq = torch.tensor(
        octave_bands(interval=interval,
...
def octave_bands(interval: int = 1,
                 start_freq: float = 31.25,
                 end_freq: float = 16000):
    r"""
...
def geq(center_freq: torch.Tensor,
        ...
        device=None):
    r"""
    Compute the second-order sections coefficents of a graphic equalizer.
    ...
    **Returns**:
    ...
    assert len(
        gain_db
    ) == num_bands, 'The number of gains must be equal to the number of frequencies.'
...
def design_geq(target_gain: torch.Tensor,
               center_freq: torch.Tensor,
               shelving_crossover: torch.Tensor,
               fs=48000):
    r"""
    ...
    assert len(target_gain) == len(
        center_freq
    ) + 2, 'The number of target gains must be equal to the number of center frequencies + 2.'
    ...
    prototype_b, prototype_a = geq(center_freq, shelving_crossover, R,
                                   prototype_gain_array, fs)
    ...
    # Optimization
    opt_gains = minimize_LBFGS(G, targetInterp, lowerBound, upperBound,
                               num_freq)
